...
function openDevToolsFor(wc, tab) {
	...
	if (tab && tab !== 'last-selected') {
		...
		setTimeout(() => {
			try { wc.devToolsWebContents?.executeJavaScript(`DevToolsAPI.showPanel("${tab}")`); }
			catch { /* DevTools not ready or API unavailable */ }
		}, 300);
	}
}
...
module.exports = class AutoDevToolsPlugin extends Plugin {
	async onload() {
		...
		if (this.settings.openOnLoad) {
			this.app.workspace.onLayoutReady(() => openDevTools(this.app, this.settings.defaultTab));

			// Open DevTools in popout windows as they're created
			this.registerEvent(
				this.app.workspace.on('window-open', () => {
					setTimeout(() => openDevToolsAllWindows(this.settings.defaultTab), 500);
				})
			);
		}
	}
...
class AutoDevToolsSettingTab extends PluginSettingTab {
	...
	display() {
		...
		new SettingGroup(containerEl)
			...
			.addSetting(s => s
				.setName('Close on plugin unload')
				.addToggle(toggle => toggle
			...
			.addSetting(s => s
				.setName('Default tab')
				.setDesc('Uses an internal API that may break in future Obsidian updates.')
